...
public class StringHelper {
	...
	/**
	 * 根据传入的参数,进行组合生成参数名称=参数值的字符串 奇数为参数名称,偶数为参数值
	 * 
	...
	public static String getParaErrorMsgInfo(String... paraNameAndValues) {
		...
		for (Integer i = 0; i < paraNameAndValues.length; i += 2) {
			buffer.append(paraNameAndValues[i]).append(StringHelper.PARA_EQUAL)
					.append(paraNameAndValues[i + 1])
					.append(StringHelper.COMMA);
		}
	...
	public static String resolveUid(String uid, String devid) {
		if (uid.length() < 17) {
			...
			int[] p_arr = {
					StringHelper.getIntegerByStr(phone.substring(0, 3))
							.intValue(),
					StringHelper.getIntegerByStr(phone.substring(3, 6))
							.intValue(),
					StringHelper.getIntegerByStr(phone.substring(6, 9))
							.intValue(),
					StringHelper.getIntegerByStr(phone.substring(9)).intValue() };

			int dlen = devid.length() - 1;
			int[] d_arr = {
					StringHelper.getIntegerByStr(devid.substring(dlen))
							.intValue(),
					StringHelper.getIntegerByStr(
							devid.substring(dlen - 1, dlen)).intValue(),
					StringHelper.getIntegerByStr(
							devid.substring(dlen - 2, dlen - 1)).intValue(),
					StringHelper.getIntegerByStr(
							devid.substring(dlen - 3, dlen - 2)).intValue() };
			String fill = "9";
			...
			for (int i = 0; i <= 3; i++) {
				...
				if (d_arr[i] == 1) {
					...
					temp = String.valueOf(p_arr[i] / d_arr[i])
							+ String.valueOf(p_arr[i] % d_arr[i]);
				}
				int len = temp.length();
	...
	public static String unresolveUid(String uid, String devid) {
		if (uid.length() < 17) {
			...
			String[] pt_arr = { phone.substring(0, 4), phone.substring(4, 8),
					phone.substring(8, 12), phone.substring(12) };
			int[] p_arr = { 0, 0, 0, 0 };
			for (int i = 0; i <= 3; i++) {
				for (int j = 3; j >= 0; j--) {
					if (pt_arr[i].charAt(j) == fill) {
						pt_arr[i] = pt_arr[i].substring(0,
								pt_arr[i].length() - 1);
					} else {
						break;
					}
			...
			for (int i = 0; i <= 3; i++) {
				...
				if (d_arr[i] == 1) {
					...
					temp = String.valueOf((p_arr[i] / 10) * d_arr[i] + p_arr[i]
							% 10);
				}
	...
	public static String getRadomSixBitNum(){
	...
	public static void main(String[] arg) {
		for(int i=0;i<6;i++){
			System.out.println(getRadomSixBitNum());
		}
